...
def _format_day_summary(items: list[dict[str, Any]]) -> str:
    ...
    rain_suffix = ""
    if max_pop >= 0.2:
        rain_suffix = f", probabilidad de lluvia {round(max_pop * 100)}%"

    return f"{day_name} {day_number}: {description.capitalize()}, {temp}°C{rain_suffix}."
...
async def get_forecast(lat: float, lon: float) -> str:
    ...
    if not forecast_items:
        return "Pronóstico no disponible para las coordenadas entregadas."

    items_by_date: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for item in forecast_items:
        forecast_dt = _parse_forecast_datetime(item)
        items_by_date[forecast_dt.date().isoformat()].append(item)

    summaries = [
        _format_day_summary(items)
